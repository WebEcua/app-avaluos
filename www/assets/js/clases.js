class Sistema {
    ...
    subirParte(datos) {
        ...
        return new Promise((resolve, reject) => {
            // Calcular checksum MD5 de la parte de la imagen
            if (datos.imagen) {
                datos.checksum = this.calcularMD5(datos.imagen);
            }
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (response.estado === "success" && response.checksum === datos.checksum) {
                        resolve(response);
                    } else {
                        alert("La coneccion con el servidor es muy inestable, por favor vuelva a intentarlo.");
                        acciones.sincronizarOriginal();
                        reject("Error en el envio de datos");
                    }
                },
                error: (err) => {
                    reject(err);
